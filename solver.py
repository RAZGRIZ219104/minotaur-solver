...
class _PymsnoHop(PolarBearRouter):
    """pymsno pymsno-hop: never-regress delta on the certified champion.
    ...
    def metadata(self):
        ...
        try:
            ...
            if _dc.is_dataclass(base):
                return _dc.replace(base, name="pymsno-hop")
        except Exception:
            pass
        rep = getattr(base, "_replace", None)
        if callable(rep):
            try:
                return rep(name="pymsno-hop")
            except Exception:
                pass
        return base
    ...
    def _py_improve(self, intent, state, snapshot, base):
        ...
        try:
            ...
            return ExecutionPlan(intent_id=intent.app_id, interactions=ix, deadline=deadline,
                                 nonce=state.nonce, metadata={"solver": "pymsno-hop", "chain_id": cid})
        except Exception:
