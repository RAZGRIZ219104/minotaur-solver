...
class _PymsnoSplit2(PolarBearRouter):
    """pymsno pymsno-split2: never-regress delta on the certified champion.
    ...
    def metadata(self):
        ...
        try:
            ...
            if _dc.is_dataclass(base):
                return _dc.replace(base, name="pymsno-split2")
        except Exception:
            pass
        rep = getattr(base, "_replace", None)
        if callable(rep):
            try:
                return rep(name="pymsno-split2")
            except Exception:
                pass
        return base
    ...
    def _py_improve(self, intent, state, snapshot, base):
        ...
        return ExecutionPlan(intent_id=intent.app_id, interactions=ix, deadline=deadline,
                             nonce=state.nonce, metadata={"solver": "pymsno-split2", "chain_id": cid})
